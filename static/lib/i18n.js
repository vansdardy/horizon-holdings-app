...
(function (root, factory) {
  ...
  const STRINGS = {
    en: {
      ...
      'hold.cashNote': '<b>The pound cash pool is larger than it should be.</b> Until v1.10.1 this app stored London prices in pence while labelling them pounds, so the index bought its UK shares in steps a hundred times too coarse and the rounding remainder piled up here: {a} against the {e} a correct allocation would have left. The money is real and the NAV counts it correctly — it is simply sitting in cash instead of shares, a drag of {p} of the fund. The annual rebalance reinvests every cash pool from scratch, and this notice disappears by itself when it does.',
    ...
    zh: {
      ...
      'hold.thActual': '权重',
      'hold.thTarget': '目标',
      'hold.cashNote': '<b>英镑现金池偏大。</b> 在 v1.10.1 之前,伦敦交易所的价格被按便士存储、却当作英镑使用,于是指数买入英股时的取整步长放大了一百倍,零头都堆在了这里:当前 {a},而正确建仓本应只剩 {e}。这笔钱是真实存在的,净值也算得没错 —— 只是本该买成股票却留作了现金,相当于 {p} 的资金闲置。每年的再平衡会把所有现金重新投入,到那时这条提示会自动消失。',
      'hold.cashTitle': '剩余现金',
      'hold.thCcy': '币种',
      'hold.thAmount': '金额',
      'hold.equity': '证券',
