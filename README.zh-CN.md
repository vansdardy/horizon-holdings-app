...
- **程序会自动检查新版本。** 启动后不久检查一次,之后每六小时一次;下载前会先问你,
  安装前会再问一次。也可以随时手动检查:**右键点托盘图标 → 检查更新…**。
  更新不会动你的数据库、设置和净值历史。

  如果你现在用的是 **v1.6.1 或更早的版本**,那些版本是在自动更新功能加进来之前打包的,
  没法自己更新——请到 [Releases 页面](https://github.com/vansdardy/horizon-holdings-app/releases)
  手动装一次,之后它就会自己保持最新。
